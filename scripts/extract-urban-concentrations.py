...
out = root / 'public/geo/urban-concentrations'
...
with (root / 'public/br_geobr_mapas_concentracao_urbana.csv').open() as source:
    for index, row in enumerate(csv.DictReader(source)):
        ...
        groups[state].append({'type': 'Feature', 'properties': {
            'id': str(index), 'concentrationId': row['id_concentracao_urbana'],
            'name': row['concentracao_urbana'], 'state': state,
...
for state, features in sorted(groups.items()):
    ...
    manifest.append({'state': state, 'count': len(features), 'url': f'/geo/urban-concentrations/{filename}'})
