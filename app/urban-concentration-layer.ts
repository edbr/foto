...
import manifest from '../public/geo/urban-concentrations/manifest.json';

export const urbanConcentrationCount = manifest.reduce((total, file) => total + file.count, 0);

export function showUrbanConcentrations(map: mapboxgl.Map, visible: boolean, t: (text: string) => string = (text) => text) {
  ...
  for (const file of manifest) {
    const id = `concentration-${file.state}`;
    if (!map.getSource(id) && visible) {
      map.addSource(id, { type: 'geojson', data: file.url, attribution: 'Urban concentrations: supplied geobr CSV; population 2010' });
      map.addLayer({
        id, source: id, type: 'fill',
        paint: { 'fill-color': '#657da4', 'fill-opacity': 0.22 },
      }, 'northeast-mask');
      map.addLayer({
        id: `${id}-outline`, source: id, type: 'line',
        paint: { 'line-color': '#465f8c', 'line-width': 1.5, 'line-opacity': 0.9 },
      }, 'northeast-mask');
    }
    ...
    const click = (event: mapboxgl.MapLayerMouseEvent) => {
      ...
      heading.textContent = properties.name || 'Urban concentration';
